...
void MFRC522::PCD_WriteRegister(PCD_Register reg, byte value) {
    std::vector<byte> sendData = {reg, value};
    m_callbackWrite(std::move(sendData));
...
void MFRC522::PCD_WriteRegister(PCD_Register reg, byte count, byte *values ) {
    ...
    std::vector<byte> sendData(count + 1);
    sendData[0] = reg;
...
byte MFRC522::PCD_ReadRegister(PCD_Register reg) {
    byte value = 0x00;
    byte sendReg = (0x80 | reg);
    std::vector<byte> readData = {sendReg, value};
    std::vector<byte> answerData = m_callbackRead(std::move(readData));
...
void MFRC522::PCD_ReadRegister(	PCD_Register reg,	///< The register to read from. One of the PCD_Register enums.
    ...
    byte sendReg = (0x80 | reg);
    std::vector<byte> readData(count + 1, sendReg);
    readData.back() = 0x00;

    std::vector<byte> answerData = m_callbackRead(std::move(readData));

    for (byte i = 0; i < answerData.size(); ++i) {
        values[i] = answerData[i];
    }
 ...
 * Beware: When two PICCs are in the field at the same time I often get STATUS_TIMEOUT_T - probably due do bad antenna design.
 *
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
...
MFRC522::StatusCode MFRC522::PICC_HaltA() {
    ...
    // We interpret that this way: Only STATUS_TIMEOUT_T is a success.
    result = PCD_TransceiveData(buffer, sizeof(buffer), nullptr, 0);
    if (result == STATUS_TIMEOUT_T) {
        return STATUS_OK;
    }
 ...
 *
 * @return STATUS_OK on success, STATUS_??? otherwise. Probably STATUS_TIMEOUT_T if you supply the wrong key.
 */
...
MFRC522::StatusCode MFRC522::PCD_MIFARE_Transceive(	byte *sendData,		///< Pointer to the data to transfer to the FIFO. Do NOT include the CRC_A.
    ...
    if (acceptTimeout && result == STATUS_TIMEOUT_T) {
        return STATUS_OK;
    }
    if (result != STATUS_OK) {
        return result;
...
const char* MFRC522::GetStatusCodeName(MFRC522::StatusCode code	///< One of the StatusCode enums.
    ...
    case STATUS_TIMEOUT_T:		return "Timeout in communication.";
...
void MFRC522::PICC_DumpMifareClassicSectorToSerial(Uid *uid,			///< Pointer to Uid struct returned from a successful PICC_Select().
    ...
    for (int8_t blockOffset = no_of_blocks - 1; blockOffset >= 0; blockOffset--) {
        ...
        if (firstInGroup) {
            // Print access bits
            std::cout << " [ ";
            std::cout << ((g[group] >> 2) & 1);
            std::cout << " ";
            std::cout << ((g[group] >> 1) & 1);
            std::cout << " ";
...
bool MFRC522::MIFARE_OpenUidBackdoor(bool logErrors) {
    ...
    if(status != STATUS_OK) {
        if(logErrors) {
            std::cout << "Card did not respond to 0x40 after HALT command. Are you sure it is a UID changeable one?" << std::endl;
            std::cout << "Error name: ";
            std::cout << GetStatusCodeName(status) << std::endl;;
        }
        return false;
    ...
    if(status != STATUS_OK) {
        if(logErrors) {
            std::cout << "Error in communication at command 0x43, after successfully executing 0x40"<< std::endl;
            std::cout << "Error name: ";
            std::cout << GetStatusCodeName(status) << std::endl;
        }
        return false;
...
bool MFRC522::MIFARE_SetUid(byte *newUid, byte uidSize, bool logErrors) {
    ...
    if (!newUid || !uidSize || uidSize > 15) {
        if (logErrors) {
            std::cout << "New UID buffer empty, size 0, or size > 15 given" << std::endl;
        }
        return false;
    }
    ...
    if (status != STATUS_OK) {
        ...
        if (status == STATUS_TIMEOUT_T) {
            ...
            if (!PICC_IsNewCardPresent() || !PICC_ReadCardSerial()) {
                std::cout << "No card was previously selected, and none are available. Failed to set UID."<< std::endl;
                return false;
            }
            ...
            if (status != STATUS_OK) {
                ...
                if (logErrors) {
                    std::cout << "Failed to authenticate to card for reading, could not set UID: "<< std::endl;
                    std::cout << GetStatusCodeName(status)<< std::endl;
                }
                return false;
            }
    ...
    if (status != STATUS_OK) {
        if (logErrors) {
            std::cout << "MIFARE_Write() failed: " << std::endl;
            std::cout << GetStatusCodeName(status) << std::endl;
        }
        return false;
    }
...
void MFRC522::setCallbackWrite(const std::function<void (std::vector<uint8_t> &&)>& callbackWrite)
{
    m_callbackWrite = callbackWrite;
}

void MFRC522::setCallbackRead(const std::function<std::vector<uint8_t>(std::vector<uint8_t> &&)>& callbackRead)
{
    m_callbackRead = callbackRead;
}

void MFRC522::initMIFARE()
{
    // TxModeReg <- 0x00
    // RxModeReg <- 0x00
    // Reset baud rates
...
//        PCD_WriteRegister(TReloadRegL, 0xE8);
    ...
    // TReloadRegH <- 0x03
    PCD_WriteRegister(TReloadRegH, 0xFF);
    // TReloadRegL <- 0xE8
    PCD_WriteRegister(TReloadRegL, 0x10);
    // TxASKReg <- 0x40
    PCD_WriteRegister(TxASKReg, 0x40);
    // ModeReg <- 0x3D
    PCD_WriteRegister(ModeReg, 0x3D);
    //  TxControlReg -> 0x80
    byte b = PCD_ReadRegister(TxControlReg);
    //  TxControlReg <- 0x83
    PCD_WriteRegister(TxControlReg, 0x83);
    //    CollReg -> 0xA0
    b = PCD_ReadRegister(CollReg);
    //    CollReg <- 0x20
    PCD_WriteRegister(CollReg, 0x20);
    //    CommandReg <- 0x00
    PCD_WriteRegister(CommandReg, 0x00);
    //    ComIrqReg <- 0x7F
    ...
    //    BitFramingReg <- 0x07
    PCD_WriteRegister(BitFramingReg, 0x07);
    //    CommandReg <- 0x0C
    PCD_WriteRegister(CommandReg, 0x0C);
    //    BitFramingReg -> 0x07
    b =  PCD_ReadRegister(BitFramingReg);
    //    BitFramingReg <- 0x87
    PCD_WriteRegister(BitFramingReg, 0x87);

    //    ComIrqReg -> 0x04

    b =  PCD_ReadRegister(ComIrqReg);
    b =  PCD_ReadRegister(ComIrqReg);
    b =  PCD_ReadRegister(ComIrqReg);
    // CommandReg <- 0x0F
    //PCD_WriteRegister(CommandReg, PCD_SoftReset); //0x0F
}

void MFRC522::readUIDCard()
{
    //  ErrorReg -> 0x00
    byte b = PCD_ReadRegister(ErrorReg);
    //  FIFOLevelReg -> 0x02
    b = PCD_ReadRegister(FIFOLevelReg);
    //  FIFODataReg -> 0x04 0x00
    std::vector<byte> answer(2);
    PCD_ReadRegister(FIFODataReg, answer.size(), answer.data(), 0);
    //  ControlReg -> 0x10
    b = PCD_ReadRegister(ControlReg);
    //  CollReg -> 0x20
    b = PCD_ReadRegister(CollReg);
    //  CollReg <- 0x20
    PCD_WriteRegister(CollReg, 0x20);
    ...
    //FIFOLevelReg <- 0x80
    PCD_WriteRegister(FIFOLevelReg, 0x80);
    // FIFODataReg <-0x93 0x20
    std::vector<byte> sendFIFODataReg = {0x93, 0x20};
    PCD_WriteRegister(FIFODataReg,sendFIFODataReg.size(),
                      sendFIFODataReg.data());
    ...
    b = PCD_ReadRegister(BitFramingReg);
    //BitFramingReg <- 0x80
    PCD_WriteRegister(BitFramingReg, 0x80);
    ...
    {
        //    ErrorReg -> 0x00
        b = PCD_ReadRegister(ErrorReg);

        //FIFOLevelReg -> 0x05
        b = PCD_ReadRegister(FIFOLevelReg);

        // FIFODataReg -> 0x1B 0x67 0x95 0x63 0x8A
        std::vector<byte> answer(b);
        PCD_ReadRegister(FIFODataReg, answer.size(), answer.data(), 0);
...
bool MFRC522::findCard()
{
    //    //    CommandReg -> 0x20
    //    byte b = PCD_ReadRegister(CommandReg);
    //    //     TxModeReg <- 0x00
    //    //     RxModeReg <- 0x00
    //    // Reset baud rates
    //    PCD_WriteRegister(TxModeReg, 0x00);
    ...
    //    PCD_WriteRegister(ModeReg, 0x3D);
    //    //  TxControlReg -> 0x80
    //    b = PCD_ReadRegister(TxControlReg);
    //    //  TxControlReg <- 0x83
    //    PCD_WriteRegister(TxControlReg, 0x83);
    ...
    //  CollReg -> 0x20
    byte b = PCD_ReadRegister(CollReg);
    //  CollReg <- 0x20
    PCD_WriteRegister(CollReg, 0x20);
    //  CommandReg <- 0x00
    PCD_WriteRegister(CommandReg, 0x00);
