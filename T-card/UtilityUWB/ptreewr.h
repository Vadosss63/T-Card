...
private:
    template<typename Type>
    void AddParameterField(const std::string& name, const Type& val, const std::string& type);
...
template<typename Type>
void PtreeWR::AddParameterField(const std::string& name, const Type& val, const std::string& type)
{
    std::string pathValue = std::string(m_parametersField) + "." + name + "." + std::string(m_valueField);
    m_treeJson.put(pathValue, val);
    std::string pathType = std::string(m_parametersField) + "." + name + "." + std::string(m_typeField);
...
{
    std::string pathToParameter = std::string(m_parametersField) + "." + nameParameter + "." + m_valueField;
