...
SenderUDP::SenderUDP(ushort port): m_sock(m_service)
{
    SetEndPoint("127.0.0.1", port);
    Open();
}

SenderUDP::SenderUDP(const std::string& adress, ushort port):
    m_sock(m_service)
{
...
void SenderUDP::SetEndPoint(const std::string& adress, ushort port) noexcept
{
...
bool SenderUDP::ReadWithTimeout(const Timeout& expiryTime)
{
    ...
    m_sizeRecivedByte = 0;
    timer.async_wait([&] (const ErrorCode& error)
    {
        timerResult.reset(error);
    });

    boost::optional<ErrorCode> readResult;

    m_sock.async_receive(boost::asio::buffer(m_bufferReciveData.Data(), m_bufferReciveData.Size()),
...
std::string SenderUDP::GetData()
{
    std::string data((char*)m_bufferReciveData.Data(), m_sizeRecivedByte);
    return data;
}



bool SenderUDP::Bind(ushort port) noexcept
{
...
ReciverUDP::ReciverUDP(): m_service(new IOservice), m_socket(*m_service),
    m_isStart(false){}

ReciverUDP::ReciverUDP(ushort port):
    m_service(new IOservice), m_socket(*m_service), m_isStart(false)
{
    SetEndPoint("127.0.0.1", port);
}

ReciverUDP::ReciverUDP(std::string adress, ushort port):
    m_service(new IOservice), m_socket(*m_service), m_isStart(false)
{
    SetEndPoint(adress, port);
}

ReciverUDP::ReciverUDP(std::string adress, ushort port, uint sizeBuf):
    m_service(new IOservice), m_socket(*m_service), m_isStart(false), m_bufferReciveData(sizeBuf)
{
    SetEndPoint(adress, port);
}
...
void ReciverUDP::SetEndPoint(const std::string& adress, ushort port) noexcept
{
    m_epReciver = EndPoint(Address_v4::from_string(adress), port);
}

bool ReciverUDP::IsStart() noexcept
{
...
std::string ReciverUDP::GetData()
{
    std::string data((char*)m_bufferReciveData.Data(), m_sizeRecivedByte);
    return data;
}

std::pair<uint8_t*, size_t> ReciverUDP::GetData_uint8()
{
    uint8_t* data = (uint8_t*)malloc(m_sizeRecivedByte);
...
{
    m_socket.async_receive_from(boost::asio::buffer(m_bufferReciveData.Data(), m_bufferReciveData.Size()), m_epSender,
                                [&] (const ErrorCode& error, size_t s)
    {
        ReadData(error, s);
    });
...
{
    ClearErrors();
    m_socket.send_to(boost::asio::buffer(data, size), m_epSender, 0, GetErrors());
    return IsNoError();
}
...
{
    ClearErrors();
    m_socket.set_option(boost::asio::ip::multicast::join_group(m_epReciver.address()), GetErrors());
    return IsNoError();
}

void ReciverUDP::CreateBuffer(uint size)
{
    m_bufferReciveData.Resize(size);
}

BufferReciveData::BufferReciveData()
{
    Resize(m_sizeBufferRecive);
}

BufferReciveData::BufferReciveData(size_t size)
{
    Resize(size);
}
...
uint8_t* BufferReciveData::Data()
{
    return m_bufferRecive;
}

size_t BufferReciveData::Size()
{
    return m_sizeBufferRecive;
}
